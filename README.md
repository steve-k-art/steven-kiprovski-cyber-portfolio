...
I build and break things. My work focuses on offensive security — finding what's exploitable before attackers do — and defensive monitoring — understanding what an attack looks like in logs, packets, and alerts. Everything in this portfolio was done hands-on in a lab I built from scratch.
...
> A structured vulnerability assessment against a simulated enterprise environment. Full-scope scanning, enumeration, and exploitation across network services and web applications to identify and validate critical security weaknesses.
...
**[→ View Case Study](https://github.com/steve-k-art/steven-kiprovski-cyber-portfolio/tree/main/vulnerability-assessment)**
...
**Red Team highlights:**
- Exploited 14 CVEs across 6 services — gained root shell via vsftpd backdoor (CVE-2011-2523) and Samba RCE (CVE-2017-7494)
- Brute-forced SMB credentials with Hydra, executed Slowloris DoS, performed XSS on OWASP JuiceShop

**Blue Team highlights:**
- Detected 944 NIDS alerts in Kibana — identified 3 missed detection gaps
- Triaged 9,115 log events across OSSEC HIDS, Snort, CRON, NTPD
- Correlated post-exploitation C2 traffic across Kibana, Squert, and Wireshark

`Kali Linux` `Metasploit` `Splunk` `Security Onion` `Wireshark` `Snort` `OSSEC` `Kibana` `Hydra` `Nmap` `OWASP ZAP`

**[→ View Case Study](https://github.com/steve-k-art/steven-kiprovski-cyber-portfolio)**
...
|-----------|-----------|----------------|
| Penetration testing | SIEM (Splunk, Kibana) | VirtualBox, VMware |
| CVE exploitation | IDS/IPS (Snort, Suricata, OSSEC) | Network segmentation, VLANs |
| Vulnerability assessment | Log analysis & triage | Firewall rules (iptables, OPNsense) |
| Web app testing (OWASP) | Packet analysis (Wireshark) | Active Directory, RBAC |
| Social engineering | Incident detection & response | MFA (Google Authenticator) |
| Report writing | Risk assessment (CVE-based) | Linux & Windows Server admin |
